...
def main(text):
    ...
    if 'list' in text:
        ...
        # see if tasks is in user command
            ...
            if not text:
                return (
                    "I need a cluster name to complete the requested operation. To view the cluster names, use 'jarvis ecs list clusters <region>'")
            ...
            if 'running' in text:
                ...
                try:
                    ...
                    print(("exception in tasks option is ", e))
                        ...
                        attachments.append(
                            {
                                ...
                                'fields': [{
                                    ...
                                    'value': str(
                                        image['taskDefinition']['containerDefinitions'][0]['memory']) + " Megabytes",
                                    'short': True
                                }, {
                                    'title': 'Running Tasks',
...
def information():
    ...
    jarvis lambda list clusters [in <region/account>] [sendto <user or channel>]
    jarvis lambda list services <cluster> [in <region/account>] [sendto <user or channel>]
    jarvis lambda describe|desc <cluster> [in <region/account>] [sendto <user or channel>]
    jarvis lambda describe|desc <service> <cluster> [in <region/account>] [sendto <user or channel>]
    jarvis lambda list tasks[---<task_name_optional>] running <cluster> [in <region/account>] [sendto <user or channel>]
