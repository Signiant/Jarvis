...
def information():
	...
	jarvis s3 list binaries <bucket> <in region/account> [sendto <user or channel>]
	jarvis s3 compare [<bucket>] within <account> with [<bucket>] within <account>  [sendto <user or channel>]"""
...
def get_in_s3_compare_data(config, args, args_eval):
	...
	for account in config:
		if result.has_key('RoleArn'):
			break
		elif result['AccountName'] == account['AccountName'] and account['Buckets'].has_key(result['region']):
		  result['RoleArn'] = account['RoleArn']
		  # set the directory
		  if account['Buckets']:
			  for region in account['Buckets']:
				  if region == result['region']:
					  if result.has_key('Bucket'):
						  ...
						  result['Directory_List'] = account['Buckets'][region]
		  break
		elif account['Buckets'].has_key(result['region']):
		  if result.has_key('Bucket'):
	...
	if result.has_key('RoleArn') == False \
            or result.has_key('RoleArn') == False \
            or not result.has_key('Accountname') == False\
            or len(result['Directory_List']) < 1:
	  result = dict()
